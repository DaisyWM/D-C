...
export default function PlacePage() {
  ...
  const checkSavedStatus = async (placeId) => {
    try {
      const response = await axios.get("/saved");
      const savedPlaces = response.data;
      const savedPlace = savedPlaces.find(
        (saved) => saved.place._id === placeId
      );
      if (savedPlace) {
        setIsSaved(true);
        setSavedId(savedPlace._id);
      }
    } catch (error) {
  ...
  async function savePlace() {
    try {
      const response = await axios.post("/saved", { place: place._id });
  ...
  async function deleteSavedPlace() {
    try {
      const response = await axios.delete(`/deleteSaved/${savedId}`);
      if (response.status === 200) {
        setIsSaved(false);
